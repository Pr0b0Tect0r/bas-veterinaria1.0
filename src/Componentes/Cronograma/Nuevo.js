...
export default function TablaMovimiento() {
	...
	const { cronograma } = React.useContext(CronogramaContext)
	const [openDialog, setOpenDialog] = React.useState(false)
	const [cronogramas, setCronogramas] = React.useState(typeof cronograma.informacion.fecha !== 'undefined' ? cronograma.informacion : {
		fecha: '',
		fechacorte: '',
		numero: '',
		observaciones: '',
	...
	const [selectedDateFiltro, setSelectedDateFiltro] = React.useState(cronogramas.fecha);
	const classes = useStyles();
	const detalles = [
		{
			item: 1,
			proveedor: 'SACO SHIPPING SAC',
			analitica: '4699315',
			td: 'ANT',
			numdoc: '3447-8',
			emision: '02/03/2020',
			vencimiento: '02/03/2020',
			frecep: '',
			centrocosto: 'IMPORTACION',
			mon: 'USD',
			saldo: '-25.00',
			monto: '-25.00',
			montosoles: '-86.28',
	...
	const consultarAcciones = () => {
		...
		dispatch(['cronogramaNuevo', '/cronograma/nuevo', 'funcion', interactions.formContent.funcionSecundaria, aja])
		// })
	}
			...
			<Grid container spacing={2}>
				...
				<Grid item xs={12} sm={4}>
					<Box borderColor='primary.main' borderRadius={5} {...defaultProps}>
						<Grid container spacing={2} style={{ padding: '2px' }}>
							<Grid item xs={12}>
								<Typography variant='body1' className={classes.texto}>{`N° 000${cronogramas.numero}`}</Typography>
							</Grid>
						</Grid>
					</Box>
				</Grid>
				<Grid item xs={12}>
					<TextField
						...
						fullWidth
						margin='normal'
						value={cronogramas.observaciones}
				...
				<Grid item xs={12} sm={2}>
					<Button variant='outlined' color='primary' className={classes.button2} onClick={() => alert('seleccionar documento')}>Documentos</Button>
				</Grid>
				<Grid item xs={12} sm={2}>
					<Button variant='outlined' className={classes.button2} color='primary' onClick={() => alert('generar')}>Generar pago</Button>
				</Grid>
				<Grid item xs={12} sm={2}>
					<Button variant='outlined' className={classes.button2} color='primary' onClick={() => alert('asignar pago')}>asignar pago</Button>
				</Grid>
				<Grid item xs={12} sm={2}>
					<Button variant='outlined' className={classes.button2} color='primary' onClick={() => alert('asignar cuenta')}>asignar cuenta</Button>
				</Grid>
				<Grid item xs={12} sm={4}>
			...
			<Zoom in={true} timeout={800}>
				<MaterialTable
					...
					columns={[
						...
						{ title: 'Analitica', field: 'analitica' },
						{ title: 'TD', field: 'td' },
						{ title: 'Num Doc.', field: 'numdoc' },
						{ title: 'Emisión', field: 'emision' },
						{ title: 'Vencimiento', field: 'vencimiento' },
						{ title: 'F. Recep.', field: 'frecep' },
						{ title: 'Centro costo', field: 'centrocosto' },
						{ title: 'Mon', field: 'mon' },
						{ title: 'Saldo', field: 'saldo' },
						{ title: 'Monto', field: 'monto' },
						{ title: 'Monto Soles', field: 'montosoles' },
						{ title: 'Monto retención', field: 'montoretencion' },
						{ title: 'Tipo pago', field: 'tipopago' },
						{ title: 'Libro', field: 'libro' },
						{ title: 'Cuenta pago', field: 'cuentapago' },
						{ title: 'Cuenta', field: 'cuenta' },
						{ title: 'Operación', field: 'operacion' },
						{ title: 'Nombre archivo', field: 'nombrearchivo' },
						{ title: 'Estado proc.', field: 'estadoproc' },
						{ title: 'Num planilla', field: 'numplanilla' },
						{ title: 'Observación arch', field: 'observacionarch' }
					]}
					data={detalles}
					localization={{
						pagination: {
